...
export default function CreateMeetingPage() {
    ...
    if (!client || !user) {
        return <Loader2 className="mx-auton animate-spin" />;
    }
    return (<div className="flex flex-col items-center space-y-6">
        <h1 className="text-2xl font-bold text-center">Welcome {user?.username}!</h1>
        <div className="w-80 mx-auto space-y-6 rounded-md bg-slate-100 p-5">
            <h2 className="font-bold text-xl text-center">Create a new meeting</h2>
            <DescriptionInput
                value={descriptionInput}
                onChange={setDecriptionInput} />
            <StartTimeInput
...
function DescriptionInput({ value, onChange }: DescriptionInputProps) {
    ...
    return <div className="space-y-2">
        ...
        {active && (
            <label className="bloack space-y-1">
...
function MeetingLink({ call }: MeetingLinkProps) {
    ...
    return <div className="text-center flex flex-col items-center gap-3">
        ...
        <a href={getMailToLink(
            meetingLink, call.state.startsAt,
